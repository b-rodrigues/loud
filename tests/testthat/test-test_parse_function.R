...
test_that("make simple command", {
  expect_equal(".l$value |> loud_exp(.log_df = .l$log_df)",
               make_command(
                 parse_function(
                   deparse1(substitute(
                     loud_exp()
...
test_that("make command with any args", {
  expect_equal(".l$value |> loud_exp(ha, hu, .log_df = .l$log_df)",
               make_command(
                 parse_function(
                   deparse1(substitute(
                   loud_exp(ha, hu)
...
test_that("make dplyr command", {
  expect_equal(".l$value |> loud_summarise(mtcars, average = mean(cyl), .log_df = .l$log_df)",
               make_command(
                 parse_function(
                   deparse1(substitute(
                   loud_summarise(mtcars, average = mean(cyl))
